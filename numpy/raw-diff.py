...
diff_32 = img2_32 - img1_32  # has range -64K to 64K-1
diff_clipped = np.clip(diff_32, -2**15, 2**15-1) ## clip to range -32K to 32K-1
diff_offset = diff_clipped + 2**15  ## shift to range 0 to 64K-1


if outfilename != None:
  print(f"Creating: {outfilename}")
  diff_offset.astype(np.uint16).tofile(outfilename)
